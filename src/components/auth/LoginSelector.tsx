  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setErrorMsg(res.error || messages.login.loginError);
      }
    } catch {
      setErrorMsg(messages.login.unexpectedError);
    } finally {
      setLoading(false);
    }
  };
      ...
      {currentUser && (
        <div className="mb-6 bg-slate-800/80 border border-sky-500/30 rounded-2xl p-4 flex items-center justify-between shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-slate-400">{messages.login.currentUser}</p>
          ...
          >
            <span>{messages.login.enterApp}</span>
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  {isWasher
                    ? messages.login.washerDescription
                    : dept.slug === 'admin'
                    ? messages.login.adminDescription
                    : messages.login.departmentDescription}
                </p>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs font-semibold text-sky-400">
                <span>{messages.login.signIn}</span>
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-3xl p-6 sm:p-8 w-full max-w-md shadow-2xl relative animate-in fade-in zoom-in duration-150">
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">{messages.login.confirmCredentials}</p>
              </div>
            </div>
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              <div>
                ...
                {!hideDefaultPins && (
                  <p className="text-[11px] text-slate-500 mt-1.5">
                    {messages.login.defaultPin} <span className="font-mono text-slate-400">{selectedDept.pin || '1234'}</span>
                  </p>
                )}
              </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                >
                  {loading ? messages.login.signingIn : messages.login.submit}
                  <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </form>
